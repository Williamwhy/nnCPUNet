...
from nncpunet.preprocessing.resampling.utils import recursive_find_resampling_fn_by_name
import nncpunet
from batchgenerators.utilities.file_and_folder_operations import load_json, join

from nncpunet.imageio.reader_writer_registry import recursive_find_reader_writer_by_name
from nncpunet.utilities.find_class_by_name import recursive_find_python_class
from nncpunet.utilities.label_handling.label_handling import get_labelmanager_class_from_plans
...
if TYPE_CHECKING:
    ...
    from nncpunet.imageio.base_reader_writer import BaseReaderWriter
    from nncpunet.preprocessing.preprocessors.default_preprocessor import DefaultPreprocessor
    from nncpunet.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
...
class ConfigurationManager(object):
    def __init__(self, configuration_dict: dict):
        ...
        if 'architecture' not in self.configuration.keys():
            warnings.warn("Detected old nnU-Net plans format. Attempting to reconstruct network architecture "
                          "parameters. If this fails, rerun nnCPUnet_plan_experiment for your dataset. If you use a "
    ...
    def preprocessor_class(self) -> Type[DefaultPreprocessor]:
        preprocessor_class = recursive_find_python_class(join(nncpunet.__path__[0], "preprocessing"),
                                                         self.preprocessor_name,
                                                         current_module="nncpunet.preprocessing")
...
class PlansManager(object):
    ...
    def experiment_planner_class(self) -> Type[ExperimentPlanner]:
        ...
        experiment_planner = recursive_find_python_class(join(nncpunet.__path__[0], "experiment_planning"),
                                                         planner_name,
                                                         current_module="nncpunet.experiment_planning")
...
if __name__ == '__main__':
    from nncpunet.paths import nnUNet_preprocessed
    from nncpunet.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name
