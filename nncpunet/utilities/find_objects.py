...
import nncpunet
from nncpunet.paths import nnUNet_extTrainer
...
def recursive_find_trainer_class_by_name(trainer_name: str):
    ...
    nnunet_trainer = recursive_find_python_class(
        join(nncpunet.__path__[0], "training", "nnUNetTrainer"),
        trainer_name,
        "nncpunet.training.nnUNetTrainer",
        nncpunet.__path__[0],
    )
    ...
    if nnunet_trainer is None:
        if nnUNet_extTrainer.is_set():
            ...
            print(
                f"Trainer '{trainer_name}' not found in nncpunet.training.nnUNetTrainer.\n"
        ...
        if nnunet_trainer is None:
            raise RuntimeError(
                ...
                f'{join(nncpunet.__path__[0], "training", "nnUNetTrainer")}). '
