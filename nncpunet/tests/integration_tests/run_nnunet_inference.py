...
def run_tests_and_exit_on_failure():
    ...
    # Copy example file
    os.makedirs("nncpunet/tests/github_actions_output", exist_ok=True)
    shutil.copy("nncpunet/tests/example_data/example_ct_sm.nii.gz", "nncpunet/tests/github_actions_output/example_ct_sm_0000.nii.gz")

    # Run nnunet
    subprocess.call("nnCPUnet_predict -i nncpunet/tests/github_actions_output -o nncpunet/tests/github_actions_output -d 300 -tr nnUNetTrainer -c 3d_fullres -f 0 -device cpu", shell=True)

    # Check if the nnunet segmentation is correct
    img_gt = nib.load("nncpunet/tests/example_data/example_ct_sm_T300_output.nii.gz").get_fdata()
    img_pred = nib.load("nncpunet/tests/github_actions_output/example_ct_sm.nii.gz").get_fdata()
    ...
    # Clean up
    shutil.rmtree("nncpunet/tests/github_actions_output")
